...
public class HibernatePuebloDAOTest  {
	...
	@Test
	public void dadoUnLugarConDosEspeciesLoPersistoYLuegoLoRecuperoParaComprobarQueSeHayaPersistidoDeManeraCorrecta() {
		Runner.runInSession(() -> {
			this.puebloDAO.savePueblo(this.pueblo);
			return null;
		});
		
	 	Lugar lugarRecuperado =	Runner.runInSession(() -> {
					 				return this.puebloDAO.getPueblo("PuebloTest");
					 			});
			
		assertEquals(this.pueblo.getNombre(), lugarRecuperado.getNombre());
		assertEquals(this.pueblo.getEspecies().size(), (int) 2);
